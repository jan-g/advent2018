...
day11 :: [String] -> IO ()
day11 ls = do
  let Just serial = parseLine (ls !! 0)
      pls = makeArray serial
      (x, y, m) = calcMax pls 3
  putStrLn $ show (x, y, m)

makeArray serial =
 Array.array ((minX, minY), (maxX, maxY))
             [((x, y), cellValue serial x y) | x <- [minX .. maxX], y <- [minY .. maxY]]

cellValue serial x y =
  let rackId = x + 10
      pl1 = rackId * y
      pl2 = pl1 + serial
      pl3 = pl2 * rackId
      pl4 = (pl3 `div` 100) `mod` 10
  in  pl4 - 5

sumCells :: Array.Array (Integer, Integer) Integer -> Integer -> Array.Array (Integer, Integer) Integer
sumCells pls n =
  Array.array ((minX, minY), (maxX - n + 1, maxY - n + 1))
              [((x, y), val x y) | x <- [minX .. maxX - n + 1], y <- [minY .. maxY - n + 1]]
  where
    val x y =
      sum [pls ! (x + dx, y + dy) | dx <- [0..n-1], dy <- [0..n-1]]

calcMax :: Array.Array (Integer, Integer) Integer -> Integer -> (Integer, Integer, Integer)
calcMax pls n =
  let totals = sumCells pls n
      (x, y, m) = maxFor [minY .. maxY - n + 1] [minX .. maxX - n + 1] totals
  in (x, y, m)

maxFor :: [Integer] -> [Integer] -> Array.Array (Integer, Integer) Integer -> (Integer, Integer, Integer)
maxFor (y:ys) (x:xs) ts =
  let (mx, my, mt) = (x, y, ts ! (x, y))
  in  scanY (y:ys) (x:xs) (mx, my, mt) ts
  where
      scanY [] xs max ts = max
      scanY (y:ys) xs max ts = scanY ys xs (scanX y xs max ts) ts
      scanX _ [] (mx, my, mt) _ = (mx, my, mt)
      scanX y (x:xs) (mx, my, mt) ts | (ts ! (x, y)) > mt = scanX y xs (x, y, ts ! (x, y)) ts
                                     | otherwise = scanX y xs (mx, my, mt) ts
...
Your puzzle input is still 1718.
-}
...
day11b ls = do
  let Just serial = parseLine (ls !! 0)
      pls = makeArray serial
      (mx, my, mt, mn) = searchForMax pls [minX .. maxX]
  putStrLn $ show (mx, my, mt, mn)

searchForMax :: Array.Array (Integer, Integer) Integer -> [Integer] -> (Integer, Integer, Integer, Integer)
searchForMax pls ns =
  searchForMax0 (1, 1, pls ! (1, 1), 1) ns
    where
        searchForMax0 :: (Integer, Integer, Integer, Integer) -> [Integer] -> (Integer, Integer, Integer, Integer)
        searchForMax0 (mx, my, mt, mn) [] = (mx, my, mt, mn)
        searchForMax0 (mx, my, mt, mn) (n:ns) =
          let (mx', my', mt') = calcMax pls n
          in  if mt' > mt
              then searchForMax0 (mx', my', mt', n) ns
              else searchForMax0 (mx, my, mt, mn) ns