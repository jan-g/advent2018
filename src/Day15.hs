module Day15
    ( day15
    , day15b
    , elf, goblin, Goon, pos, hp, str
    , Cave, walls
    , move, attack, adjacent, neighbours
    , demoCave
    , runUntilDead
    , flood, backtrack
    ) where
...
In the above scenario, the Elf has three targets (the three Goblins):
    ...
    Of those squares, four are reachable (marked @); the other two (on the right) would require moving through a wall
    or unit to reach.
...
data Cave = Cave { walls :: Set.Set (Integer, Integer)
                 }
  deriving (Show, Eq)
...
data Goon = Goon { species :: Species
                 , hp :: Integer
                 , str :: Integer
                 , pos :: (Integer, Integer)
                 , num :: Integer
...
readingOrder (x1, y1) (x2, y2) = compare (y1, x1) (y2, x2)

instance Ord Goon where
  compare Goon { pos=p1 } Goon { pos=p2 } = readingOrder p1 p2

elf x y n = Goon { species=Elf, hp=200, str=3, pos=(x, y), num=n }
goblin x y n = Goon { species=Goblin, hp=200, str=3, pos=(x, y), num=n }

parse :: [String] -> (Cave, Set.Set Goon, Set.Set Goon)
parse ls =
  let walls0 = Set.empty :: Set.Set (Integer, Integer)
      gobs0 = Set.empty :: Set.Set Goon
      elves0 = Set.empty :: Set.Set Goon
      (wallsN, gobsN, elvesN, _) = foldl parseLine (walls0, gobs0, elves0, 0) $ zip [0..] ls
  in  (Cave { walls=wallsN }, gobsN, elvesN)
  where
    parseLine (walls, gobs, elves, n) (y, line) = foldl (parseCol y) (walls, gobs, elves, n) $ zip [0..] line
    parseCol y (walls0, gobs0, elves0, n) (x, c) =
      case c of
      '.' -> (walls0, gobs0, elves0, n)
      '#' -> let walls1 = Set.insert (x, y) walls0
             in  (walls1, gobs0, elves0, n)
      'G' -> let gobs1 = Set.insert (goblin x y n) gobs0
             in  (walls0, gobs1, elves0, n+1)
      'E' -> let elves1 = Set.insert (elf x y n) elves0
             in  (walls0, gobs0, elves1, n+1)
      otherwise -> error ("unknown character " ++ [c])


day15 :: [String] -> IO ()
day15 ls = do
  let (cave, gobs, elves) = parse ls
  (iterZ, caveZ, gobZ, elfZ) <- runUntilDead 0 cave gobs elves
  putStrLn $ show gobs
  putStrLn $ show elves
  putStrLn $ show caveZ
  putStrLn $ show gobZ
  putStrLn $ show elfZ
  summarise iterZ gobZ elfZ

summarise :: Integer -> Set.Set Goon -> Set.Set Goon -> IO ()
summarise i gs es = do
  let hpg = sum $ map hp $ Set.toList gs
      hpe = sum $ map hp $ Set.toList es
      hps = hpg + hpe
  putStrLn $ "Combat ends after " ++ (show i) ++ " full rounds"
  putStrLn $ (if hpe == 0 then "Goblins" else "Elves") ++ " win with " ++ (show hps) ++ " total hit points left"
  putStrLn $ "Outcome: " ++ (show i) ++ " * " ++ (show hps) ++ " = " ++ (show $ hps * i)

runUntilDead iter cave gobs elves = do
  putStrLn $ show iter
  putStrLn $ showCave cave gobs elves

  if Set.null gobs || Set.null elves
  then return (iter, cave, gobs, elves)
  else do

    let mobs = Set.toAscList $ Set.union gobs elves
        (fullRound, g', e') = runRound gobs elves mobs
    putStrLn $ "Goblins -> " ++ (show $ sum $ map hp $ Set.toList g') ++ " " ++ (show g')
    putStrLn $ "Elves -> " ++ (show $ sum $ map hp $ Set.toList e') ++ " " ++ (show e')

    if Set.null g' || Set.null e'
    then do
      if fullRound
      then do
        putStrLn "Combat ends on the last turn of a full round"
      else do
        putStrLn "Combat ends partway through a round"
      return (iter + (if fullRound then 1 else 0), cave, g', e')
    else runUntilDead (iter + 1) cave g' e'
  where
    runRound :: Set.Set Goon -> Set.Set Goon -> [Goon] -> (Bool, Set.Set Goon, Set.Set Goon)
    runRound gs es [] = (True, gs, es)
    runRound gs es (m:ms)
      | Set.null gs = (False, gs, es)
      | Set.null es = (False, gs, es)
      | otherwise =
        let friends = if species m == Elf then es else gs
            enemies = if species m == Elf then gs else es
            (m's, friends') = Set.partition (((num m) ==) . num) friends
        in  {- move - or stay still if adjacent already -}
            if Set.null m's
            then runRound gs es ms
            else
              let m' = move cave friends' enemies (head $ Set.toList m's)
                  friends'' = Set.insert m' friends'
              {- attack nearby enemies -}
                  nearby = Set.filter (adjacent m') enemies
                  enemies' = if not $ Set.null nearby
                             then attack m' enemies
                             else enemies
              in  if species m == Elf
                  then runRound enemies' friends'' ms
                  else runRound friends'' enemies' ms

adjacent :: Goon -> Goon -> Bool
adjacent (Goon { pos=(x1,y1) }) (Goon { pos=(x2,y2) }) = abs(x2 - x1) + abs(y2 - y1) == 1

at p1 (Goon { pos=p2 }) = p1 == p2

move :: Cave -> Set.Set Goon -> Set.Set Goon -> Goon -> Goon
move (Cave { walls=walls }) obstacles targets (m@Goon { pos=(x,y) }) =
  let nearby = Set.filter (adjacent m) targets
      blocks = Set.unions [walls, Set.map pos obstacles, Set.map pos targets]
      goals = foldMap (neighbours . pos) targets
  in  if not $ Set.null nearby
      then m
      else
        let outPaths@(border:rest) = flood blocks goals [Set.singleton (x,y)]
            encounters = Set.intersection border goals
        in  if Set.null encounters
            then m
            else
              let chosenEncounter = Set.singleton $ head $ List.sortBy readingOrder $ Set.toList encounters
                  (_:steps:_) = backtrack [chosenEncounter] rest
                  (c:_) = List.sortBy readingOrder $ Set.toList steps
              in  m { pos=c }

attack :: Goon -> Set.Set Goon -> Set.Set Goon
attack m enemies =
  let nearby = Set.filter (adjacent m) enemies
  in  if Set.null nearby
      then enemies
      else
        let (n:ns) = List.sortBy (\ n1@Goon {hp=hp1, pos=(x1,y1)} n2@Goon {hp=hp2, pos=(x2,y2)} ->
                                    compare (hp1, y1, x1) (hp2, y2, x2)) $ Set.toList nearby :: [Goon]
            ns' = Set.delete n enemies
            hp' = (hp n) - (str m)
        in  if hp' > 0
            then Set.insert n { hp=hp' } ns'
            else ns'

neighbours (x,y) = Set.fromList [(x, y-1), (x-1, y), (x+1, y), (x, y+1)]

flood :: Set.Set (Integer, Integer) -> Set.Set (Integer, Integer) -> [Set.Set (Integer, Integer)] -> [Set.Set (Integer, Integer)]
flood obstacles targets horizons@(h:hs) =
  let expanse = foldMap neighbours h :: Set.Set (Integer, Integer)
      expanse' = Set.difference expanse obstacles
      expanse'' = Set.difference expanse' (Set.unions horizons)
  in  if Set.null expanse''
      then horizons
      else if not $ Set.null $ Set.intersection expanse'' targets
      then (expanse'':horizons)
      else flood obstacles targets (expanse'':horizons)

backtrack paths [] = paths
backtrack (horizon:rest) (p:ps) =
  let expanse = foldMap neighbours horizon
      trimmed = Set.intersection p expanse
  in  backtrack (trimmed:horizon:rest) ps


showCave cave gobs elves =
  let minY = minimum [y | (x,y) <- Set.toList $ walls cave ]
      maxY = maximum [y | (x,y) <- Set.toList $ walls cave ]
      minX = minimum [x | (x,y) <- Set.toList $ walls cave ]
      maxX = maximum [x | (x,y) <- Set.toList $ walls cave ]
  in  unlines $ showLine [minY .. maxY] [minX..maxX] (walls cave) gobs elves
  where
    showLine [] _ _ _ _ = []
    showLine (y:ys) xs c g e =
      let (map, hps) = showCol y xs c g e
      in  (map ++ "  " ++ hps):(showLine ys xs c g e)

    showCol y [] _ _ _ = ("", "")
    showCol y (x:xs) c g e =
      let wall = Set.member (x,y) c
          gob = Set.filter (at (x,y)) g
          elf = Set.filter (at (x,y)) e
          dot = if wall
                then '#'
                else if not $ Set.null gob
                then 'G'
                else if not $ Set.null elf
                then 'E'
                else '.'
          (dots, mobs) = showCol y xs c g e
          mob = if not $ Set.null gob
                then ("G(" ++ (show $ hp $ head $ Set.toList gob) ++ ") ")
                else if not $ Set.null elf
                then ("E(" ++ (show $ hp $ head $ Set.toList elf) ++ ") ")
                else ""
      in (dot:dots, mob ++ mobs)
...
day15b ls = do
  let (cave, gobs, elves) = parse ls
  (iterZ, caveZ, gobZ, elfZ, power) <- searchForNoElfLosses [4..] cave gobs elves
  putStrLn $ show caveZ
  putStrLn $ show gobZ
  putStrLn $ show elfZ
  putStrLn $ "Power boost needed was to " ++ (show power)
  summarise iterZ gobZ elfZ

searchForNoElfLosses (p:ps) cave gobs elves = do
  let elves' = Set.map (\g -> g { str=p }) elves
  (iterZ, caveZ, gobZ, elfZ) <- runUntilDead 0 cave gobs elves'
  if Set.size elfZ == Set.size elves
  then return (iterZ, caveZ, gobZ, elfZ, p)
  else searchForNoElfLosses ps cave gobs elves