...
tree kids meta = Tree kids meta


parseLine :: String -> Maybe Tree
parseLine s =
  case readP_to_S treeParser s of
    [] -> Nothing
    [(e, "")] -> Just e

treeParser :: ReadP Tree
treeParser = do
  numKids <- intParser
  char ' '
  numMeta <- intParser
  kids <- count (fromIntegral numKids) (do
            char ' '
            child <- treeParser
            return child)
  meta <- count (fromIntegral numMeta) (do
            char ' '
            meta <- intParser
            return $ fromIntegral meta)
  return $ Tree kids meta


day8 :: [String] -> IO ()
day8 ls = do
  let Just input = parseLine $ ls !! 0
      result = sumMeta input
  putStrLn $ show result


sumMeta :: Tree -> Int
sumMeta (Tree kids meta) = (sum meta) + (sum $ map sumMeta kids)
...
evaluate (Tree [] meta) = sum meta
evaluate (Tree kids meta) = sum [if 1 <= n && n <= (length kids) then evaluate (kids !! (n-1)) else 0 | n <- meta]